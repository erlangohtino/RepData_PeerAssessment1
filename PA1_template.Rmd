...
`r mymissing` missing values in dataset. The mean of the steps per day, estimating the NA values in the dataset as the mean number of steps corresponding to the corresponding 5-min interval in the day is `r mynewmean` and the median is `r mynewmedian`. They are slightly higher than the original values `r mymean` and `r mymedian`.
